...
function Label({ item, align }: { item: SlideItem; align: 'left' | 'right' }) {
    return (
        <div className={`absolute bottom-10 ${align === 'left' ? 'left-6' : 'right-6'} flex items-center gap-2 px-4 py-2 rounded-full backdrop-blur-md border bg-black/55 border-white/25 text-gray-200`}>
            <Shirt size={12} />
            <span className="text-xs font-bold uppercase tracking-widest">Apparel</span>
        </div>
    );
}

// ─── Main Hero ─────────────────────────────────────────────────────────────────
export default function AIApparelHero() {
        ...
        >
            ...
            <div className="absolute inset-0 z-0 hidden md:flex">
                ...
                <div className="relative w-1/2 h-full overflow-hidden" style={{ zIndex: 1 }}>
                    ...
                    >
                        <Image src={leftImg.src} alt="AI Apparel photography" fill className={`object-cover ${leftImg.pos}`} sizes="50vw" priority />
                    ...
                    >
                        <Image src={nextLeftImg.src} alt="AI Apparel photography" fill className={`object-cover ${nextLeftImg.pos}`} sizes="50vw" />
                        <div className="absolute bottom-0 inset-x-0 h-44 bg-gradient-to-t from-dark-bg to-transparent" />
                ...
                <div className="relative w-1/2 h-full overflow-hidden" style={{ zIndex: 1 }}>
                    ...
                    >
                        <Image src={rightImg.src} alt="AI Apparel photography" fill className={`object-cover ${rightImg.pos}`} sizes="50vw" priority />
                        <div className="absolute bottom-0 inset-x-0 h-44 bg-gradient-to-t from-dark-bg to-transparent" />
                    ...
                    >
                        <Image src={nextRightImg.src} alt="AI Apparel photography" fill className={`object-cover ${nextRightImg.pos}`} sizes="50vw" />
                        <div className="absolute bottom-0 inset-x-0 h-44 bg-gradient-to-t from-dark-bg to-transparent" />
            ...
            <div className="absolute inset-0 z-0 md:hidden">
                {SEQUENCE.map((slide, i) => (
                    ...
                    >
                        <Image
                            src={slide.src}
                            alt="AI apparel product photography"
                            fill
            ...
            <div className="relative w-full max-w-7xl mx-auto px-6 pt-28 pb-28" style={{ zIndex: 10 }}>
                <div className="max-w-xl space-y-7 animate-in fade-in slide-in-from-bottom-5 duration-1000">
                    ...
                    <p className="sr-only">
                        Create fashion and apparel product photos without a studio. Generate AI model photos, lookbooks, and marketplace-ready images for your brand.
                    </p>

                    <p className="text-lg text-gray-300 max-w-lg leading-relaxed">
                        Create fashion and apparel product photos without booking a model — AI-powered images for clothing, ethnic wear, western wear, and all your fashion pieces.
                    </p>
                    ...
                    <p className="text-sm text-gray-400 max-w-lg leading-relaxed">
                        From sarees and kurtas to shirts and dresses — generate clean, realistic product images ready for your website, ads, and online marketplaces.
                    </p>
                        ...
                        <div className="flex -space-x-2">
                            {SEQUENCE.slice(0, 3).map((img, i) => (
                                <div key={i} className="w-7 h-7 rounded-full border-2 border-dark-bg overflow-hidden relative">
                                    <Image src={img.src} alt="Fashion Brand" fill className="object-cover" sizes="28px" />
                                </div>
                            ))}
                        </div>
                        <span>Trusted by <strong className="text-white">1000+</strong> fashion brands</span>
                    </div>
                </div>
            </div>
